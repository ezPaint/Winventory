...
//            sendRedirect(request, response, "/winventory/software?key=" + key + "&deleted=" + name);
//
//        }
        ...
        if (button_param.equals("Update")){
            ...
            try {
                ...
                for (int i = 0; i < changes.size(); i += 3){
                    message += "<br>" + changes.get(i) + " was changed from " + changes.get(i + 1) + " to " + changes.get(i + 2) + ".";
                }
                
                //Record event
                EventBo.getInstance().createSystemEvent(name + " was updated: " + message,
                    getUserInfo(request), EventType.SYSTEM, software);            
            } catch (BoException e) {
                logError(log, e);
            }
            sendRedirect(request, response, "/winventory/software/view?key=" + key + "&success=" + name);
        }
    }
}
